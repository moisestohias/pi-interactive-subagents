...
* `subagent_agents: scout, researcher` — grants the child the spawning toolset (`subagent`, `subagent_message`, `subagents_list`) and restricts it to spawning only the listed agents. Omitted or empty means the child cannot spawn at all (how `scout` / `researcher` work).
