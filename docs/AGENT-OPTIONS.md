...
* `raw-artifact: true` — raw task delivery is the default (Proposal A, `file-wrapper-core-suggestions.md`): the launch command carries pi core `--disable-file-wrapper`, so the child receives the task artifact with no `<file>` wrapper. Set `raw-artifact: false` to opt out back to wrapped delivery; the resolved choice is snapshotted in the loadout and replayed on resume.
