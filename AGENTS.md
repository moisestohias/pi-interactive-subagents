...
`pi-extension/subagents/index.ts` (~320 lines) is thin wiring only: tool/command/renderer registration + `session_start`/`shutdown` delegation to `lifecycle.ts`. Pure policy
goes in the home module; session-lifecycle wiring (watchers, reload recovery,
artifact sweep, steer fan-out) lives in `lifecycle.ts`. `index.ts` keeps a delegating wrapper only when `__test__` compat
needs it (`index.ts:__test__` — `test/test.ts` imports internals through it; new suites import homes directly).

| Concern | Canonical home |
|---|---|
| Agent profiles (frontmatter, discovery tiers, spawn gate, tool→extension map) | `pi-extension/subagents/agents.ts` (frontmatter parsed once to a `Map` via `parseFrontmatterBlock`, `getFrontmatterValue` wrapper kept; single `resolveLaunchPolicy` table with `resolveEffectiveSessionMode` / `resolveLaunchBehavior` / `resolveEffectiveInteractive` deprecated aliases) |
| Launch/resume command pieces (env/cd/preamble/sandbox/allowlist/prompts) | `pi-extension/subagents/launch.ts` (`buildPiLaunchPlan` / `buildPiResumePlan` single pipeline, one env order; `launch-types.ts` deleted — import `agents.ts` directly; `cli/claude.ts` for the Claude path: `buildClaudeCommand`, sentinel (`createClaudeSentinelFile`, `0600`) + allowlisted transcript copy) |
| Live identity (running runs, kept tabs, spawn reservations) | `pi-extension/subagents/store.ts` (`SubagentStore` + singleton; `RunningSubagent` is an alias of `RunningEntry` — one interface, no boundary casts) |
| Parent-bound steer messages | `pi-extension/subagents/notifications.ts` (sole `sendMessage` owner) |
| Widget rendering | `pi-extension/subagents/widget.ts`; formatters in `format.ts`; message presentation in `renderers.ts`; result wording in `results.ts` |
...
| Session files | `pi-extension/subagents/session/` via the `session.ts` barrel (`io`, `index-cache` — quarantined, do not build on it, `registry` — `version` marker + corrupt backups, `loadout` — `isValidSubagentLoadout` refuses bad snapshots, `seed`, `stats` — `summarizeEntriesStats` for single-read completions, `sidecars` — dependency-free `atomicWriteJson` / `claimFile` / `readJsonClaim` / `takeSidecar` claiming **both** `.exit` and `.done` (child-safe; `writeAskSignalAtomic` kept as alias), `types`; dead branch ops quarantined in `legacy-branch.ts` — do not use) |
...
| Session lifecycle (launch/watch orchestration, recovery, sweep, per-session UI, ask queue) | `pi-extension/subagents/lifecycle.ts` (`launchSubagent` / `watchSubagent` / `launchAndWatch`, `teardownSession`, `recoverSession`, `resurrectRunningTab` / `decideResurrectAction`, `sweepStaleArtifacts`, `sessionCtxs`, `promotePendingAskFile` / `restoreAskClaimNoClobber`, `handleSessionStart` / `handleSessionShutdown`) |
| Tool handlers (`subagent` / `subagent_message` executes) | `pi-extension/subagents/handlers/` (`spawn.ts`, `message.ts` — thin; `steerSubagent` / `steerSubagentAsync` sharing `flattenSteerMessage`; `list.ts` — `subagents_list` + `/subagent` command, moved as-is; `validators.ts` — pure `validateSpawnParams` / `decideSteerResume`, directly unit-tested) |
| Process-global keys + import-time rotation | `pi-extension/subagents/runtime.ts` (`WIDGET` / `STATUS_INTERVAL_KEY`, `POLL_ABORT_KEY`, `RUNNING_CHILDREN_COUNT_KEY`; free of `agents.ts` imports) |
...
7. **Names are unique per spawner session** (running + reserved + registry; explicit names dedupe `X` → `X-2` via `uniqueRunningName` in `pi-extension/subagents/lifecycle.ts` / `names.ts`, keyed per-session by `keptKey` in `store.ts`). Resume reserves the name *before* the stale-sidecar unlink + shell-ready delay and re-checks after (never double-open one `.jsonl`). Never steer by `Array.find` — use `resolveRunningByName` (`store.ts`, re-exported by `lifecycle.ts`) so ambiguity errors. Never resume into a live tab (two writers corrupt one `.jsonl`); steer instead.
...
- **New tool handler** → pure gates in `handlers/validators.ts` (direct unit tests) + thin execute in `handlers/<name>.ts` calling `lifecycle.ts` orchestration; wire up in `index.ts` registration only.
- **New launch-derived agent field** → add to the `resolveLaunchPolicy` table in `agents.ts` (keep the three deprecated aliases delegating).
...
- **New sidecar/claim suffix** → add to the protocol-registry comment in `lifecycle.ts` + `docs/HOW-IT-WORKS.md` signals section; teach `sweepStaleArtifacts` (`lifecycle.ts`) whether it is swept or kept; implement via `session/sidecars.ts` primitives.
- **New corrupt-drop path** → route through `logCorruptDrop` (kind + path + reason) so the counter test covers it.
- **New notification shape** → `pi-extension/subagents/notifications.ts` helper + renderer in `renderers.ts`; keep `{triggerTurn: true, deliverAs: "steer"}` in the helper, data at call sites.
